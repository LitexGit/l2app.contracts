...
contract MultiSigWallet {
    ...
    uint256 public ethBlockNumber;
    // txhash => is proposed
    mapping (bytes32 => bool) public proposedTxMap;
    ...
    {
        transactionId = addTransaction(txHash, ethBlockNumber, destination, value, data);
        ...
        notExecuted(transactionId)
    {
        if (isConfirmed(transactionId)) {
            ...
            txn.executed = true;
            if (proposedTxMap[txn.txHash] == false
    ...
    /// @param data Transaction data payload.
    /// @return Returns transaction ID.
    function addTransaction(bytes32 txHash, uint256 ethBlockNumber, address destination, uint value, bytes memory data)
        internal
        notNull(destination)
        returns (uint transactionId)
    {
