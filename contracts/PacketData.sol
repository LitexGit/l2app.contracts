...
// source: contracts/proto/packet.proto
...
library PacketData {
    ...
    struct InitSession {
        address provider;   // tag: 1
        bytes32 gameID;   // tag: 2
        bytes32 sessionID;   // tag: 3
        bytes32 prHash;   // tag: 4
    } // end struct InitSession

    function decInitSession(bytes memory raw) internal pure returns (InitSession memory m) {
        ...
        while (buf.hasMore()) {
            ...
            if (false) {} // solidity has no switch/case
                ...
                m.provider = pb._address(buf.decBytes());
            }
            else if (tag == 2) {
                m.gameID = pb._bytes32(buf.decBytes());
            }
            else if (tag == 3) {
                m.sessionID = pb._bytes32(buf.decBytes());
            }
            else if (tag == 4) {
    ...
    } // end decoder InitSession

    struct UserRandomHash {
        address user;   // tag: 1
        bytes32 sessionID;   // tag: 2
        bytes32 rHash;   // tag: 3
    ...
    function decUserRandomHash(bytes memory raw) internal pure returns (UserRandomHash memory m) {
        ...
        while (buf.hasMore()) {
            ...
            if (false) {} // solidity has no switch/case
                ...
                m.user = pb._address(buf.decBytes());
            }
            else if (tag == 2) {
                m.sessionID = pb._bytes32(buf.decBytes());
            }
            else if (tag == 3) {
                m.rHash = pb._bytes32(buf.decBytes());
            }
            else { buf.skipValue(wire); } // skip value of unknown tag
        }
    ...
    function decUserRevealRandom(bytes memory raw) internal pure returns (UserRevealRandom memory m) {
        ...
        while (buf.hasMore()) {
            ...
            if (false) {} // solidity has no switch/case
                ...
                m.random = pb._bytes32(buf.decBytes());
            }
            else { buf.skipValue(wire); } // skip value of unknown tag
        }
