...
// source: contracts/proto/transfer.proto
...
library TransferData {
    ...
    struct Transfer {
        ...
        uint256 balance;   // tag: 2
        uint256 nonce;   // tag: 3
        uint256 amount;   // tag: 4
        bytes32 additionalHash;   // tag: 5
        bytes signature;   // tag: 6
    } // end struct Transfer

    function decTransfer(bytes memory raw) internal pure returns (Transfer memory m) {
        ...
        while (buf.hasMore()) {
            ...
            if (false) {} // solidity has no switch/case
                ...
                m.balance = pb._uint256(buf.decBytes());
            }
            else if (tag == 3) {
                m.nonce = pb._uint256(buf.decBytes());
            }
            else if (tag == 4) {
                m.amount = pb._uint256(buf.decBytes());
            }
            else if (tag == 5) {
                m.additionalHash = pb._bytes32(buf.decBytes());
            }
            else if (tag == 6) {
                m.signature = bytes(buf.decBytes());
            }
            else { buf.skipValue(wire); } // skip value of unknown tag
        }
    } // end decoder Transfer
